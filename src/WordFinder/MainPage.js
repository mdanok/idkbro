...
function MainPage() {
  ...
  const handleButtonClick = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://apiforarabic.vercel.app/v1/wordfinder?letters=${value}&page=1${flink}&dictionary=${selectedOption}`
      );
  ...
  const handleLoadMore = async (key) => {
    try {
      ...
      const response = await fetch(
        `https://apiforarabic.vercel.app/v1/wordfinder?letters=${inputValue}&length=${key}&page=${
          pages[key] + 1
